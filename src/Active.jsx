    ...
    return (
        <div className=' w-full mx-auto max-w-xl bg-white p-4 rounded-xl shadow-md mb-6'>
            <div className='flex flex-row text-indigo-600'>
                ...
                <h1 className='font-semibold text-lg'>Active Players</h1>
            </div>
            <div className='grid grid-flow-row grid-cols-2 gap-4'>
                {active.map((list, index) =>
                (<div className={` ${userId === list.id ? "border-2 border-double border-indigo-400" : ""} flex flex-row justify-between ${index + 1 === 1 ? "bg-blue-200" : "bg-blue-50"} justify-items items-center shadow-md rounded-xl p-3 m-2`}>
                    <p className={`mr-2 ${index + 1 === 1 ? "font-bold  " : ""}`}>{index + 1}</p>
                    <p className={`text-gray-800 ${index + 1 === 1 ? "font-bold " : ""}`} key={list.id}>{list.name}</p>
                    {userId === list.id && (<p className=' animate-pulse ml-1 text-xs bg-indigo-500 text-white px-2 py-0.5 rounded'>You</p>)}
